...
data_movies_test = pd.read_csv('tmdb_5000_movies_testing_classification.csv')
data_credits_test = pd.read_csv('tmdb_5000_credits_test.csv')

test_data = pd.merge(data_movies_test,data_credits_test,  left_on='id', right_on='movie_id',how="left")
...
for i in test_df.axes[0]:
    ...
    if str(test_df.loc[i,'crew']) == 'nan':
        test_df.loc[i,'crew'] = "[]"

    if str(test_df.loc[i,'cast']) == 'nan':
        test_df.loc[i,'cast'] = "[]"
...
principalComponents_test = pca.transform(tmp_test)
...
if (os.path.exists('finalsvm_PCA.pkl')):
    svm_model_linear_ovr = joblib.load('finalsvm_PCA.pkl')
else:
    s=time.time()
    ...
    joblib.dump(svm_model_linear_ovr, 'finalsvm_PCA.pkl')
...
if (os.path.exists('finalknn_PCA.pkl')):
    knn = joblib.load('finalknn_PCA.pkl')
else:
    s=time.time()
    knn.fit(X_train, Y)
    e = time.time()
    models.append('knn')
    train_time_with_PCA.append(e - s)
    joblib.dump(knn, 'finalknn_PCA.pkl')
...
if (os.path.exists('finalc.pkl')):
    clf_joblib = joblib.load('finalc.pkl')
else:
    s=time.time()
    ...
    train_time_with_PCA.append(e - s)
    joblib.dump(clf_joblib, 'finalc.pkl')
...
if (os.path.exists('finaladaboos_PCA.pkl')):
    bdt = joblib.load('finaladaboos_PCA.pkl')
else:
    s = time.time()
    bdt.fit(X_train, Y)
    e = time.time()
    ...
    train_time_with_PCA.append(e - s)
    joblib.dump(clf_joblib, 'finaladaboos_PCA.pkl')
...
if (os.path.exists('finalRandom_forst_PCA.pkl')):
    reg = joblib.load('finalRandom_forst_PCA.pkl')
else:
    s = time.time()
    reg.fit(X_train, Y)
    e = time.time()
    ...
    train_time_with_PCA.append(e - s)
    joblib.dump(clf_joblib, 'finalRandom_forst_PCA.pkl')
...
if (os.path.exists('finalLogistic_PCA.pkl')):
    theta = joblib.load('finalLogistic_PCA.pkl')
else:
    s = time.time()
    theta = fit(X, Y)
    e = time.time()
    ...
    train_time_with_PCA.append(e - s)
    joblib.dump(clf_joblib, 'finalLogistic_PCA.pkl')
...
if (os.path.exists('finalsvm.pkl')):
    svm_model_linear_ovr = joblib.load('finalsvm.pkl')
else:
    s=time.time()
    svm_model_linear_ovr.fit(X, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(svm_model_linear_ovr, 'finalsvm.pkl')
...
if (os.path.exists('finalknn.pkl')):
    knn = joblib.load('finalknn.pkl')
else:
    s=time.time()
    knn.fit(X_train, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(knn, 'finalknn.pkl')
...
# Load the model from the file
if (os.path.exists('finalDecision_tree.pkl')):
    D_tree = joblib.load('finalDecision_tree.pkl')
else:
    s=time.time()
    D_tree.fit(X_train, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(D_tree, 'finalDecision_tree.pkl')
# Use the loaded model to make predictions
...
if (os.path.exists('finalAdaboost_tree.pkl')):
    bdt = joblib.load('finalAdaboost_tree.pkl')
else:
    s=time.time()
    bdt.fit(X_train, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(bdt, 'finalAdaboost_tree.pkl')
...
if (os.path.exists('finalRandom_forst.pkl')):
    reg = joblib.load('finalRandom_forst.pkl')
else:
    s = time.time()
    reg.fit(X_train, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(reg, 'finalRandom_forst.pkl')
...
sns.heatmap(confusion_matrix(Y_test,y_pred,labels=[0,1,2]),annot=True)
print(confusion_matrix(Y_test,y_pred))

#sk = (Y_test,y_pred,labels=[0,1,2])
...
if (os.path.exists('finallogistic.pkl')):
    theta = joblib.load('finalRandom_forst.pkl')
else:
    theta = fit(X, Y)
    e = time.time()
    train_time_without_PCA.append(e - s)
    joblib.dump(theta, 'finallogistic.pkl')

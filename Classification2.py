...
if (os.path.exists('c.pkl')):
    clf_joblib = joblib.load('c.pkl')
else:
    s=time.time()
...
s=time.time()
svm_model_linear_ovr = OneVsRestClassifier(SVC(kernel='rbf', C=1)).fit(X, Y)
e=time.time()
